...
if args.checkBins:
    ...
    for ib in list(range(len(tnpBins['bins']))):
...
if args.createHists:
    for sampleType in tnpConf.samplesDef.keys():
        ...
        info = {
            'infile'  : sample['path'],
            'outfile' : '%s/%s_%s.root' % ( tnpConf.baseOutDir ,sample['name'], list(tnpConf.flag.keys())[0] ),
            #'tree'    : '%s/fitter_tree'% tnpConf.tnpTreeDir,
            'tree'    : '%s'% tnpConf.tnpTreeDir,
            'weight'  : tnpConf.weightName,
            'flag'    : tnpConf.flag[list(tnpConf.flag.keys())[0]],
            'cut'     : sample['cut'],
...
if  args.doFit:
    ...
    info = {
        'infile'  : '%s/%s_%s.root' % ( tnpConf.baseOutDir ,  sample['name'], list(tnpConf.flag.keys())[0] ),
        #'tree'    : '%s/fitter_tree'% tnpConf.tnpTreeDir,
        'tree'    : '%s'% tnpConf.tnpTreeDir,
        'mcTruth' : sample['mcTruth'],
        'isMC'    : isMC,
    ...
    info['mcRef'] = '%s/%s_%s.root' % ( tnpConf.baseOutDir ,  sampleMC['name'], list(tnpConf.flag.keys())[0] )
...
if  args.doPlot:
    ...
    if sample['nEvts'] < 0 : isMC = False
    info = {
        'outfile' : '%s/%s_%s.%s.root' % ( tnpConf.baseOutDir,sample['name'], list(tnpConf.flag.keys())[0],fitType ),
        'mcTruth' : sample['mcTruth'],
        'isMC'    : isMC,
        'plotDir' : plottingDir
    }        
...
if args.sumUp:
    info = {
        'dataNominal' : '%s/%s_%s.%s.root' % ( tnpConf.baseOutDir , tnpConf.samplesDef['data' ]['name'], list(tnpConf.flag.keys())[0], 'nominalFit' ),
        'dataAltSig'  : '%s/%s_%s.%s.root' % ( tnpConf.baseOutDir , tnpConf.samplesDef['data' ]['name'], list(tnpConf.flag.keys())[0], 'altSigFit'  ),
        'dataAltBkg'  : '%s/%s_%s.%s.root' % ( tnpConf.baseOutDir , tnpConf.samplesDef['data' ]['name'], list(tnpConf.flag.keys())[0], 'altBkgFit'  ),
        'mcNominal'   : '%s/%s_%s.root' % ( tnpConf.baseOutDir , tnpConf.samplesDef['mcNom']['name'], list(tnpConf.flag.keys())[0] ),
        'mcAlt'       : None,
        'tagSel'      : None
        }

    if not tnpConf.samplesDef['mcAlt' ] is None:
        info['mcAlt'    ] = '%s/%s_%s.root'  % ( tnpConf.baseOutDir , tnpConf.samplesDef['mcAlt' ]['name'] , list(tnpConf.flag.keys())[0] )
    if not tnpConf.samplesDef['tagSel'] is None:
        info['tagSel'   ] = '%s/%s_%s.root'  % ( tnpConf.baseOutDir , tnpConf.samplesDef['tagSel']['name'] , list(tnpConf.flag.keys())[0] )
