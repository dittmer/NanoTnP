...
struct config_t {
  // basic                                                                                                                                                                                                
  bool isMC=false;
  std::string mcweight="XSWeight*puWeight*TriggerEffWeight_2l*EMTFbug_veto*PrefireWeight*METFilter_MC"; //nominal
  std::string lumi;
  std::string jsonFile;
  ...
  std::string TagCandidate = "Electron_pt>35 && Electron_cutBased==4 && abs(Electron_eta)<= 2.1 && !(abs(Electron_eta)>= 1.4442 && abs(Electron_eta)<=1.566)";
