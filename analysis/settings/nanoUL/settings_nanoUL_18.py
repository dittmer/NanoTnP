...
flags = {
    ...
    'passingttHMVAUL'    : '({0}) && ({1}) && ({2}) && ({3}) && (Probe_mvaFall17V2Iso_WP90==1) && (Probe_mvaTTHUL>0.87)'.format(cutDxy,cutDz,cutLoose,cutIso),
    'passingttHMVAULSS'  : '({0}) && ({1}) && ({2}) && ({3}) && (Probe_mvaFall17V2Iso_WP90==1) && (Probe_mvaTTHUL>0.87) && (Probe_3charge)'.format(cutDxy,cutDz,cutLoose,cutIso),
    'passingHWWMVA'      : '({0}) && ({1}) && ({2}) && ({3}) && (Probe_mva>0.868)'.format(cutDxy,cutDz,cutLoose,cutIso)
}

baseOutDir = '%s/analysis/results/nanoUL_18/tnpEleID' %os.environ.get('BASETNP')
...
samplesDef = {
    'data'   : tnpSamples.nanoUL_18['data_Run2018'].clone(),
    'mcNom'  : tnpSamples.nanoUL_18['DYJetsToLL_M-50-LO'].clone(),
    'mcAlt'  : tnpSamples.nanoUL_18['DYJetsToLL_M-50'].clone(),
    'tagSel' : tnpSamples.nanoUL_18['DYJetsToLL_M-50-LO'].clone(),
}
...
if not samplesDef['tagSel'] is None:
    ...
    samplesDef['tagSel'].set_cut('Tag_pt > 37') #nominal 34
...
tnpParNomFit = [
    ...
    "acmsF[60.,50.,80.]","betaF[0.03,0.02,0.07]","gammaF[0.1, -2, 2]","peakF[90.0]",
    ]

tnpParAltSigFit = [
    "meanP[-0.0,-5.0,5.0]","sigmaP[1,0.7,6.0]","alphaP[2.0,1.2,3.5]" ,'nP[3,-5,5]',"sigmaP_2[1.5,0.5,6.0]","sosP[1,0.5,5.0]",
    #"meanF[-0.0,-5.0,5.0]","sigmaF[2,0.7,15.0]","alphaF[2.0,1.2,3.5]",'nF[3,-5,5]',"sigmaF_2[2.0,0.5,6.0]","sosF[1,0.5,5.0]",
    "meanF[-5.0,-10.0,0.0]","sigmaF[2,0.7,15.0]","alphaF[2.0,1.2,3.5]",'nF[3,-5,5]',"sigmaF_2[2.0,0.5,6.0]","sosF[1,0.5,6.0]",
    "acmsP[60.,50.,75.]","betaP[0.04,0.01,0.06]","gammaP[0.1, 0.005, 1]","peakP[90.0]",
    "acmsF[60.,50.,75.]","betaF[0.015,0.01,0.06]","gammaF[0.05, 0.005, 1]","peakF[90.0]",
    ]
