...
flags = {
    ...
    'passingttHMVAUL'    : '({0}) && ({1}) && ({2}) && ({3}) && (Probe_mvaFall17V2Iso_WP90==1) && (Probe_mvaTTH>0.7)'.format(cutDxy,cutDz,cutLoose,cutIso),
    'passingttHMVAULSS'  : '({0}) && ({1}) && ({2}) && ({3}) && (Probe_mvaFall17V2Iso_WP90==1) && (Probe_mvaTTH>0.7) && (Probe_3charge)'.format(cutDxy,cutDz,cutLoose,cutIso),
    'passingHWWMVA'      : '({0}) && ({1}) && ({2}) && ({3}) && (Probe_mva>0.868)'.format(cutDxy,cutDz,cutLoose,cutIso)
}

baseOutDir = '%s/analysis/results/nanoUL_17BCDE/tnpEleID' %os.environ.get('BASETNP')
...
samplesDef = {
    'data'   : tnpSamples.nanoUL_17['data_Run2017BCDE'].clone(),
...
tnpParAltSigFit = [
    ...
    "meanF[-0.0,-10.0,5.0]","sigmaF[2,0.7,15.0]","alphaF[2.0,1.2,3.5]",'nF[3,-5,5]',"sigmaF_2[2.0,0.5,6.0]","sosF[1,0.4,8.0]",
    "acmsP[60.,50.,75.]","betaP[0.04,0.01,0.06]","gammaP[0.1, 0.005, 1]","peakP[90.0]",
    "acmsF[60.,50.,75.]","betaF[0.02,0.01,0.06]","gammaF[0.06, 0.005, 1]","peakF[90.0]",
    ]
